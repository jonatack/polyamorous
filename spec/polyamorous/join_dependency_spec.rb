...
module Polyamorous
  describe JoinDependency do
    ...
    context 'with symbol joins' do
      ...
      specify { subject.join_associations.should be_all {
        |a| a.join_type == Arel::InnerJoin }
      }
    end
    ...
    context 'with outer join' do
      subject {
        new_join_dependency(
          Person, new_join(:articles, :outer)
        )
      }
      specify { subject.join_associations.should have(1).association }
      specify { subject.join_associations.should be_all {
    ...
    context 'with nested outer joins' do
      subject {
        new_join_dependency(
          Person, new_join(:articles, :outer) => new_join(:comments, :outer)
        )
      }
      specify { subject.join_associations.should have(2).associations }
      specify { subject.join_associations.should be_all {
    ...
    context 'with polymorphic belongs_to join and nested symbol join' do
      subject {
        new_join_dependency(
          Note, new_join(:notable, :inner, Person) => :comments
        )
      }
